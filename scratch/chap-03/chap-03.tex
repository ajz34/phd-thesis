...
\begin{itemize}[nosep]
  ...
  \item 双电子积分能量 $E^\textsf{ERI} [\mathbf{D}]$：
        \begin{equation}
          E^\textsf{ERI} [\mathbf{D}] = E^\textsf{J} [\mathbf{D}] + c_\mathrm{x} E_\mathrm{x}^\textsf{exact} [\mathbf{D}] + E_\mathrm{x}^\textsf{LR} [\mathbf{D}]
        \end{equation}
  ...
  \item DFA 格点积分能量 $E^\textsf{DFA} [\mathbf{D}]$\footnote{区别于 DFA 近似总能量 $E_\mathrm{tot}$。} 用以处理“Jacob 阶梯”上 1--3 阶 (LDA, GGA, meta-GGA) 交换相关能；它可以表示为对空间的积分
        \begin{equation}
...
\end{equation}
其中，$E^\textsf{core} [\mathbf{D}]$ 与 $E^\textsf{non-elec}$ 对于能量泛函和自洽场泛函没有区别。微扰能 $E^\textsf{PT} [\mathbf{C}]$ 仅在能量泛函出现；出于行文便利，将不对其作 $\mathrm{n}$ 的上标区分。
...
\end{equation}
它是自洽场作为变分方法的重要中间量。自洽场能量 $E^\textsf{SCF}$ 对占据轨道 $C_{\mu i}$ 的导数为
\begin{align}
...
\end{equation}
该张量具有与 ERI 张量 $(\mu \nu | \kappa \lambda)$ 相同的 8 重对称性；其对称性的来源是被求导量 $D_{\mu \nu}$ 与 $D_{\kappa \lambda}$ 作为密度矩阵本身的对称性、以及两次偏导数可交换的对称性。上式的 4 倍是闭壳层实轨道 $C_{\mu p}^* = C_{\mu p}$ 下乘积因子；该因子将使得原子轨道基下 Fock 矩阵对分子轨道系数的导数具有比较简单的形式：
\begin{equation}
  \frac{\partial F_{\mu \nu}}{\partial C_{\kappa i}} = \frac{\partial F_{\mu \nu}}{\partial D_{\eta \lambda}} \frac{\partial D_{\eta \lambda}}{\partial C_{\kappa i}} = 4 \frac{\partial F_{\mu \nu}}{\partial D_{\kappa \lambda}} C_{\lambda i} = A_{\mu \nu, \kappa \lambda} C_{\lambda i} = A_{\mu \nu, \kappa i}
\end{equation}
...
\begin{equation}
  ...
  = F_{\mu \nu}^\mathbb{A} + \frac{\partial F_{\mu \nu}}{\partial C_{\kappa i}} \partial_\mathbb{A} C_{\mu i}= F_{\mu \nu}^\mathbb{A} + A_{\mu \nu, mi} U_{mi}^\mathbb{A}
\end{equation}
...
\begin{itemize}[nosep]
  ...
  \item 对于含有多种 MP2 相关能形式的泛函 (以 lrc-XYG3、RS-PBE-P86 为代表)，其微扰能形式是
        ...
        \end{equation*}
        即微扰总 Lagrangian 的贡献等于各子项的 Lagrangian、依其对能量贡献的比例的加权和。这种可加性也同样可以应用到交换相关泛函的子项拆分；以及弛豫密度、Z 矩阵、A 张量等也适用这种可加性。同时，除了使用了不同类型但维度相同的 ERI，短程、长程 MP2 相关能计算方式与普通的 MP2 完全一致；因此，不失一般性，我们仅讨论 $E^\textsf{PT} = c_\mathrm{c}^\textsf{(2)} E_\mathrm{c}^\textsf{(2)}$ 的情形。
  \item 对于含有多种严格交换能的泛函 (以 $\omega$B97X-2(TQZ)、RS-PBE-P86 为代表)，首先短程交换能 $E_\mathrm{x}^\textsf{SR}$ 可以拆分为普通的严格交换能 $E_\mathrm{x}^\textsf{exact}$ 与长程交换能 $E_\mathrm{x}^\textsf{LR}$ 的差减；其次，长程交换能的计算与普通严格交换能计算流程一致。与 MP2 相关能同样，我们也仅在后文引入严格交换能 $E_\mathrm{x}^\textsf{exact}$。
