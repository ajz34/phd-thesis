...
本工作所涉及的记号、重要公式与约定俗成，在术语缩写对照表与附录 \alertref{sec.3.notations} 节中定义。本论文正文将基于这些定义展开。本工作涉及到的基组与程序、波函数方法、密度泛函近似，分别列于附录 \alertref{sec.app.dft}、\alertref{sec.app.wft}、\alertref{sec.app.basis}。
...
\begin{align}
  ...
  \hat V_\textmt{ext} &= \sum_{i}^{n_\mathrm{elec}} v (\bm{r}_i) \\
...
\begin{itemize}[nosep]
  ...
  \end{equation}
  而又由于外势 $v(\bm{r})$ 与基态电子云密度 $\rho(\bm{r})$ 有一一对应关系，因此基态能量 $E[v]$ 是关于基态电子云密度 $\rho(\bm{r})$ 的泛函，即几台能量也可以以记号 $E[\rho]$ 表示。该理论因此得名为密度泛函理论。
...
\begin{equation}
  E_\textmt{xc}^{\textmt{bDH}, \mathrm{s}} = c_\textmt{x} E_\textmt{x}^\textmt{exact} + (1 - c_\textmt{x}) E_\textmt{x}^\textmt{low-rung} + (1 - c_\textmt{c}) E_\textmt{c}^\textmt{low-rung}
\end{equation}
而总能量泛函是在自洽场泛函的基础上引入 MP2 型相关能：
\begin{equation}
  E_\textmt{xc}^\textmt{bDH} = E_\textmt{xc}^{\textmt{bDH}, \mathrm{s}} + c_\textmt{c} E_\textmt{c}^\textmt{MP2}
\end{equation}
