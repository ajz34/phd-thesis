...
\begin{table}[ht]
\centering
\caption{本工作中使用的电子结构方法及其对应极化率的记号\textrm{\textsuperscript{\emph{a}}}。}
\label{tab.5.1}
\widetabular{
    \begin{tabular}{ll}
    \toprule
...
在本工作中，我们将使用 Dunning 系列基组 aV$X$Z 与 aCV$X$Z。H 与 Br 原子并不出现在 aCV$X$Z 系列基组中；对于这两个原子作 aCV$X$Z 级别计算时，将不加说明地替换为对应的 aV$X$Z 基组。RI 近似可以大幅减少 MP2 的计算耗时，并且不会产生严重的误差损耗，是性价比相当高的技术手段\cite{Vahtras-Feyereisen.CPL.1993}。本工作的解析极化率计算中，我们对 Weigend 算法实现的 RI-JK\cite{Weigend-Weigend.PCCP.2002} 与 RI-MP2 相关贡献\cite{Weigend-Haettig.JCP.2002}计算均使用 ETB 自动辅助基\cite{Stoychev-Neese.JCTC.2017}；等比参数设为 \textsc{PySCF} 程序默认的 $\beta = 2$。由于基于 \textsc{PySCF} 的扩展程序 \textsc{dh} 的高效率实现、以及对大基组下内存的合理控制，使得本工作中所有体系都可以在 MP2/aCV5Z 下完成计算。作为自洽场的 RI-JK 方法的能量收敛精度为 $10^{-10}$ Hartree。

\subsection{数值极化率}

在第 \alertref{sec.3.title} 章中，我们已经了解极化率是对外电场强度的二阶梯度；依外电场 $\pmb{\mathcal{E}}$ 在三个空间方向取向的大小 $(\mathcal{E}_x, \mathcal{E}_y, \mathcal{E}_z)$，极化率张量 $\bm{\alpha}$ 是 $3 \times 3$ 的对称矩阵：
\begin{equation}
...
\begin{table}[!ht]
\centering
\caption{14 个小体系同性极化率的相对方均根误差\textrm{\textsuperscript{\emph{a}}}。}
\label{tab.5.2}
\widetabular{
...
\begin{table}[!ht]
\centering
\caption{7 个自旋非极化小体系异性极化率的相对方均根误差\textrm{\textsuperscript{\emph{a}}}。}
\label{tab.5.3}
\widetabular{
    \begin{tabular}{cld{2.4}d{2.4}d{2.4}d{2.4}}
    \toprule
          &              & \multicolumn{4}{c}{RelRMSD / \%}                \\ \cline{3-6}
    基组基数 & \multicolumn{1}{c}{基组}    &
...
\begin{table}[htp]
\centering
\caption{HF、MP2、CCSD 方法同性极化率的测评\textrm{\textsuperscript{\emph{a}}}。误差以相对方均根误差统计。}
\label{tab.5.7}
\widetabular{
...
\begin{table}[htp]
\centering
\caption{HF、MP2、CCSD 方法异性极化率的测评\textrm{\textsuperscript{\emph{a}}}。误差以相对方均根误差统计。}
\label{tab.5.8}
\widetabular{
    \begin{tabular}{ld{2.4}d{2.4}d{2.4}}
    \toprule
...
\begin{table}[!ht]
\centering
\caption{HR46 与 T144 子集下同性极化率 $\symup{\Delta} \tilde \alpha_\textsf{CCSD}$ 在不同 FPA 模型下的相对方均根误差\textrm{\textsuperscript{\emph{a}}}。}
\label{tab.5.s5}
\widetabular{
    \begin{tabular}{ccccc}
    \toprule
...
\begin{table}[!ht]
\centering
\caption{HR46 与 T144 子集下异性极化率 $\symup{\Delta} \tilde \gamma_\textsf{CCSD}$ 在不同 FPA 模型下的相对方均根误差\textrm{\textsuperscript{\emph{a}}}。}
\label{tab.5.s6}
\widetabular{
    \begin{tabular}{ccccc}
    \toprule
...
\begin{table}[ht]
\centering
\caption{HR45 数据集同性极化率因数值导数而起的相对方均差 (RelRMSD)\textrm{\textsuperscript{\emph{a}}}。}
\label{tab.5.s8}
\widetabular{
...
\begin{figure}[!ht]
    ...
    \caption{$\symup{\Delta} \alpha_\textsf{D(T)}$ 对体系 \ce{^3O2} 计算结果的基组收敛趋势}
    \label{fig.O2-iso}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption{$\symup{\Delta} \alpha_\textsf{D(T)}$ 对体系 \ce{^3SO} 计算结果的基组收敛趋势}
    \label{fig.SO-iso}
\end{figure}
...
\begin{figure}[!ht]
    ...
    \caption{$\symup{\Delta} \alpha_\textsf{D(T)}$ 对体系 \ce{Cl2} 计算结果的基组收敛趋势}
    \label{fig.Cl2-iso}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{$\symup{\Delta} \gamma_\textsf{D(T)}$ 对体系 \ce{^3O2} 计算结果的基组收敛趋势}
    \label{fig.O2-aniso}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{$\symup{\Delta} \gamma_\textsf{D(T)}$ 对体系 \ce{^3SO} 计算结果的基组收敛趋势}
    \label{fig.SO-aniso}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{$\symup{\Delta} \gamma_\textsf{D(T)}$ 对体系 \ce{HCHS} 计算结果的基组收敛趋势}
...
\begin{figure}[ht]
    ...
    \caption{$\symup{\Delta} \gamma_\textsf{D(T)}$ 对体系 \ce{Cl2} 计算结果的基组收敛趋势}
    \label{fig.Cl2-aniso}
\end{figure}
