...
对于式 (\ref{eq.2.GL2-W})，从更直观的角度来看，$\mathcal{W}_{\mathrm{c}, \lambda} [\rho]$ 作为关于 $\lambda$ 的函数，其在 $\lambda = 0$ 处的曲线斜率恰是 $2 E_\textmt{GL}^{(2)}$；同时参考图 \ref{fig.2.adiabatic-curve}。而若进一步假设曲线 $\mathcal{W}_{\mathrm{c}, \lambda} [\rho]$ 关于 $\lambda$ 近似为直线，那么相关能就可以近似为 GLPT2 微扰能，即 $E_\mathrm{c} [\rho] \simeq E_\textmt{GL}^{(2)}$。
...
理论上，我们可以获得依式 (\ref{eq.2.e-c-lambda}) 定义的 $\lambda=0$ 处各阶微扰能量；从而在 $\lambda$ 的级数收敛区间内给出最终的 $E_{\mathrm{c}, \lambda} [\rho]$。显然，若可以求得更高阶数的 G\"orling-Levy 微扰能 $E_\textmt{GL}^{(n)}$ ($n > 2$)，那么相关能的结果就更为精确。但如果对 G\"orling-Levy 微扰作更多分析，那么算符 $\hat H_\lambda$ 对耦合常数 $\lambda$ 的展开表达式为
\begin{equation}
...
\widetabular{
  ...
  \midrule
  $a_1$ 或 $c_\mathrm{x}$ & $E_\mathrm{x}^\textmt{exact}$ 系数   &  0.851546   &  0.872368     &  0.716712    \\
  $a_2$\tnote{a}          & $E_\mathrm{x}^\textmt{S}$ 系数       & [0.209516]  & [0.297015]    & [0.272940]   \\
  $a_3$                   & $E_\mathrm{x}^\textmt{B88}$ 系数     & -0.061062   & -0.169383     &  0.010348    \\
  $a_4$                   & $E_\mathrm{c}^\textmt{VWN3}$ 系数    &  0.168713   &  0.334350     &  0.359041    \\
  $a_5$                   & $E_\mathrm{c}^\textmt{LYP}$ 系数     &  0.204008   &  0.088983     &  0.247560    \\
  $a_6$                   & $E_\mathrm{c}^\textmt{OS-PT}$ 系数     &  0.460703   &  0.479659     &  0.286591    \\
  $a_7$                   & $E_\mathrm{c}^\textmt{SS-PT}$ 系数     &  0.214325   & -0.004645     &  0.102393    \\
  $a_8$                   & $E_\mathrm{c}^\textmt{EP}$ 占比 &  0.596938   &  1.421052     &  1.108974    \\
  $E_\mathrm{c}^\textmt{EP}$ & IEPA 类型 & \multicolumn{1}{c}{$E_\mathrm{c}^\textmt{MP2/cr}$} & \multicolumn{1}{c}{$E_\mathrm{c}^\textmt{sIEPA}$} & \multicolumn{1}{c}{$E_\mathrm{c}^\textmt{IEPA}$} \\
  \bottomrule
  \end{tabular}
}{
...
\widetabular{
  ...
  \item[a] 为方便排版，表头第二行指代的是 XYG6+1 模型泛函所使用的 EP 型相关能 $E_\mathrm{c}^\textmt{EP}$ 的具体类型；即 MP2/cr 指代 XYG6+1/cr、sIEPA 指代 XYG6+1/sIEPA、IEPA 指代 XYG6+1/IEPA。
}
\end{table}
...
\widetabular{
  ...
  \item[a] 为方便排版，表头第二行指代的是 XYG6+1 模型泛函所使用的 EP 型相关能 $E_\mathrm{c}^\textmt{EP}$ 的具体类型；即 MP2/cr 指代 XYG6+1/cr、sIEPA 指代 XYG6+1/sIEPA、IEPA 指代 XYG6+1/IEPA。后续表格类同。
}
\end{table}
